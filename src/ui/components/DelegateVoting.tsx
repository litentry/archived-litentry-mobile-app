...
import {useBottomSheetInternal, Button, Select, Text} from '@ui/library';
...
export function DelegateVoting({fromAccount, onClose}: DelegateVotingProps) {
  ...
  return (
    <Layout>
      <Text variant="titleMedium" style={globalStyles.textCenter}>{`Delegate voting`}</Text>
      <Padder />
            ...
            {(() => {
              switch (page.type) {
                ...
                case 'SELECT_AMOUNT':
                  return (
                    <View style={styles.pagerItem}>
                      <Text variant="labelMedium">{`Conviction`}</Text>
                      <Padder scale={0.5} />
                      <Select
                        items={convictions ?? []}
                ...
                case 'PREVIEW':
                  return (
                    <View style={[styles.pagerItem, globalStyles.alignCenter]}>
                      <Row label={'Delegate'}>
                        <Text variant="titleSmall">{formatBalance(delegateAmountBn, {isShort: true})}</Text>
                      </Row>
                      <Row label={'Conviction'}>
                        <Text variant="titleSmall">{conviction?.text}</Text>
                      </Row>
...
function Row({label, children}: {label: string; children: React.ReactNode}) {
  return (
    <View style={styles.row}>
      <Text variant="titleSmall" style={styles.rowLabel}>
        {label}:
      </Text>
