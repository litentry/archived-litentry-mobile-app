...
import {Button, useTheme, Text, Icon, Modal} from '@ui/library';
  ...
  return (
    <SafeView edges={noTopEdges}>
          ...
          {proposal.seconds.length > 0 ? (
            <>
              ...
              <TouchableOpacity onPress={toggleSecondsVisibility} style={globalStyles.rowAlignCenter}>
                <Text variant="titleMedium">{`Endorsed (${proposal.seconds.length})`}</Text>
      ...
      <Modal visible={endorseModalOpen} onDismiss={resetEndorseModal}>
        <Text variant="bodySmall">{`Endorse with account`}</Text>
        <SelectAccount
          onSelect={(account) => {
